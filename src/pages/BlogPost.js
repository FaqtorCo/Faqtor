        ...
        {content.sections &&
          content.sections.map((section, index) => (
            <div key={index} className="mb-12">
              ...
              {section.additionalCode && (
                <div className="my-8">
                  <div className="bg-gray-900 rounded-lg p-6 border border-gray-700">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-sm text-[#DAF7A6] font-medium uppercase tracking-wider">
                        {section.additionalCode.language}
                      </span>
                    </div>
                    <pre className="text-gray-300 overflow-x-auto">
                      <code>{section.additionalCode.code}</code>
                    </pre>
                  </div>
                </div>
              )}
              ...
              {/* Testing Examples */}
              {section.testingExamples && (
                <div className="my-8">
                  <h4 className="text-xl font-semibold text-[#DAF7A6] mb-4">
                    API Testing Examples:
                  </h4>
                  <div className="space-y-4">
                    {section.testingExamples.map((example, idx) => (
                      <div
                        key={idx}
                        className="bg-gray-900 rounded-lg p-4 border border-gray-700"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm font-medium text-[#DAF7A6] uppercase tracking-wider">
                            {example.method}
                          </span>
                        ...
                        <code className="text-gray-300 block mb-2">
                          {example.url}
                        </code>
                        {example.body && (
                          <div className="mt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {section.resources.map((resource, idx) => (
                      <a
                        ...
                        className="block bg-gray-900 rounded-lg p-4 border border-gray-700 hover:border-[#DAF7A6] transition-all duration-300 hover:-translate-y-1 no-underline"
                      >
                        <h5 className="text-[#DAF7A6] font-semibold mb-2 no-underline">
                          {resource.title}
                        </h5>
                        <p className="text-gray-300 text-sm no-underline">
                          {resource.description}
                        </p>
                        <div className="flex items-center mt-2 text-xs text-gray-400 no-underline">
                          <svg
  ...
  return (
    <div className="bg-black min-h-screen text-white">
      ...
      <article className="container mx-auto px-4 py-16 sm:px-6 lg:px-8">
        ...
        {/* Blog Content */}
        <div className="max-w-4xl mx-auto">{renderContent(blog.content)}</div>
        ...
        {relatedBlogs.length > 0 && (
          <div className="max-w-4xl mx-auto mt-16">
            <h3 className="text-2xl font-bold text-[#DAF7A6] mb-8">
              Related Articles
            </h3>
