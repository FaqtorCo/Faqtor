// First, create a new component for the floating chatbot widget
...
const ChatbotWidget = () => {
  ...
  const sessionIdRef = useRef(localStorage.getItem('faqtor-chat-session') || crypto.randomUUID());
      ...
      for (const mutation of mutations) {
        if (mutation.addedNodes.length || 
            (mutation.type === 'attributes' && mutation.target.classList && 
             Array.from(mutation.target.classList).some(c => c.includes('n8n')))) {
          shouldCheck = true;
          break;
        }
      }
  ...
  useEffect(() => {
    ...
    return () => {
      if (!isOpen && chatInstanceRef.current && typeof chatInstanceRef.current.destroy === 'function') {
        try {
  ...
  const initializeChat = () => {
    try {
      ...
      const existingChats = document.querySelectorAll('.n8n-chat, [id^="n8n-chat"]');
      ...
      const chat = createChat({
        ...
        styles: {
          chatWindow: 'display: none !important; visibility: hidden !important;',
          ...
          onResponse: (response) => {
            ...
            if (response) {
              ...
              if (response.text) {
                ...
                setMessages(prev => [...prev, { type: 'bot', content: response.output }]);
...
const sendMessageDirectly = async (text) => {
    try {
      ...
      const response = await fetch('https://n8n.softtik.com/webhook/9c494346-6972-4f49-a50f-3631792000ac/chat', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      {isOpen ? (
        <div className="chat-window bg-white rounded-lg shadow-xl w-80 md:w-96 h-96 flex flex-col overflow-hidden border-2" 
             style={{ 
               borderColor: '#DAF7A6'
             }}>
          {/* Chat Header */}
          <div className="p-3 text-white flex justify-between items-center neon-header" 
               style={{ 
            ...
            <div className="flex items-center">
              <div className="rounded-full p-1 mr-2" 
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" style={{ color: '#DAF7A6' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                </svg>
              </div>
              <h3 className="font-bold neon-text" style={{ color: '#000' }}>Chat with Max</h3>
            </div>
            <button 
              onClick={toggleChat}
          ...
          {/* Chat Messages */}
          <div className="flex-1 overflow-y-auto p-4" style={{ backgroundColor: '#f8fdf0' }}>
            {messages.map((message, index) => (
              <div 
                key={index} 
                className={`mb-4 message-animation ${message.type === 'user' ? 'flex justify-end' : 'flex justify-start'}`}
              >
                <div 
                  ...
                  style={{
                    backgroundColor: message.type === 'user' ? '#DAF7A6' : '#e8f5d0',
                    color: '#000',
            ...
            {isLoading && (
              <div className="flex justify-start mb-4">
                <div className="px-4 py-2 rounded-lg rounded-bl-none flex items-center"
                     style={{
          ...
          {/* Chat Input */}
          <form onSubmit={handleSubmit} className="border-t-2 p-3 flex" style={{ borderColor: '#DAF7A6' }}>
            <input
              type="text"
              value={inputValue}
        ...
        <div className="relative">
          ...
          <div className="question-cloud absolute -top-16 -left-8 bg-white px-2 py-4 rounded-full shadow-lg border-2"
               style={{ 
                 borderColor: '#DAF7A6',
                 backgroundColor: '#f8fdf0'
               }}>
            ...
            {/* Speech bubble tail */}
            <div className="cloud-tail absolute top-full left-1/2 transform -translate-x-1/2"
                 style={{
                   width: 0,
                   height: 0,
      ...
      <style jsx>{`
        ...
        .neon-header {
          box-shadow: 
            inset 0 0 10px rgba(218, 247, 166, 0.3),
        ...
        .dot-typing {
          ...
          color: #DAF7A6;
          box-shadow: 9984px 0 0 0 #DAF7A6, 9999px 0 0 0 #DAF7A6, 10014px 0 0 0 #DAF7A6;
        ...
        @keyframes dot-typing {
          ...
          16.667% {
            box-shadow: 9984px -5px 0 0 #DAF7A6, 9999px 0 0 0 #DAF7A6, 10014px 0 0 0 #DAF7A6;
          }
          33.333% {
            box-shadow: 9984px 0 0 0 #DAF7A6, 9999px 0 0 0 #DAF7A6, 10014px 0 0 0 #DAF7A6;
          }
          50% {
            box-shadow: 9984px 0 0 0 #DAF7A6, 9999px -5px 0 0 #DAF7A6, 10014px 0 0 0 #DAF7A6;
          }
          66.667% {
            box-shadow: 9984px 0 0 0 #DAF7A6, 9999px 0 0 0 #DAF7A6, 10014px 0 0 0 #DAF7A6;
          }
          83.333% {
            box-shadow: 9984px 0 0 0 #DAF7A6, 9999px 0 0 0 #DAF7A6, 10014px -5px 0 0 #DAF7A6;
          }
          100% {
            box-shadow: 9984px 0 0 0 #DAF7A6, 9999px 0 0 0 #DAF7A6, 10014px 0 0 0 #DAF7A6;
          }
      ...
      <div 
        ...
        className="hidden"
        style={{ display: 'none', position: 'absolute', visibility: 'hidden', opacity: 0, height: 0, width: 0, overflow: 'hidden' }}
      />
    </div>
  );
};
